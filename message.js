    ...
    Encrypter.prototype._transform = function(dump, encoding, done) {
      ...
      while (dump.length !== 0) {
        ...
        if (this.integrous === 'asym') {
          ...
          for (k in _ref1) {
            v = _ref1[k];
            footer.sigs[k] = v.toString('base64');
          }
        }
        if (this.keys["public"] != null) {
          ...
          for (k in _ref3) {
            v = _ref3[k];
            footer.keys[k] = v.toString('base64');
          }
        }
        ...
        if (footer.keys instanceof Array) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            ...
            for (n in _ref1) {
              privKey = _ref1[n];
              try {
                key = privKey.decrypt(tag, 'base64');
              } catch (_error) {
                err = _error;
                key = null;
              }
          ...
          for (_j = 0, _len1 = keys.length; _j < _len1; _j++) {
            k = keys[_j];
            try {
              key = this.keys["private"][k].decrypt(footer.keys[k], 'base64');
            } catch (_error) {
              err = _error;
              key = null;
            }
        ...
        if (footer.sigs instanceof Array) {
          ...
          for (_m = 0, _len4 = keys.length; _m < _len4; _m++) {
            ...
            try {
              v = ursa.createVerifier('sha512');
              v.update(data);
              ok = v.verify(this.keys["public"][k], footer.sigs[k], 'base64');
              if (ok) {
                break;
              }
            } catch (_error) {
